...
enum pl_tone_map_mode {
    ...
    // Guarantees no clipping but has a tendency to warp colors.
    PL_TONE_MAP_MAX,
...
struct pl_color_map_params {
    ...
    // be used. The default function is pl_tone_map_bt2390.
    //
    ...
    // Tone mapping LUT size. Defaults to 128. Note that when combining
    // this with peak detection, the resulting LUT is actually squared, so
    // avoid setting it too high.
    int lut_size;

    // Adds extra headroom when tone mapping in order to prevent clipping as a
    // result of color space reduction. It's highly recommended to leave this
    // enabled, in addition to forwarding the correct mastering metadata.
    bool gamut_headroom;
...
#define PL_COLOR_MAP_DEFAULTS                                   \
    ...
    .tone_mapping_function  = &pl_tone_map_bt2446a,             \
    .tone_mapping_mode      = PL_TONE_MAP_LUMA,                 \
