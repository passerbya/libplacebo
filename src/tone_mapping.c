...
{
    bool can_inverse = params->function->map_inverse;
    if (params->input_max > params->output_max + 1e-4) {
        params->function->map(lut, params);
    } else if (params->input_max < params->output_max - 1e-4 && can_inverse) {
...
static inline float bt1886_eotf(float x, float min, float max)
{
    ...
    return a * powf(fmaxf(x + b, 0.0f), 2.4f);
}
...
static inline float bt1886_oetf(float x, float min, float max)
{
    ...
    return fmaxf(powf(x / a, 1/2.4f) - b, 0.0f);
}
